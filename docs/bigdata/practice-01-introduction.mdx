...
Подготовим Python-окружение, повторим базовые конструкции языка и начнём исследовать табличный датасет с помощью pandas и scikit-learn.

## После материала вы сможете

- декомпозировать небольшую задачу на функции;
- работать со списками и словарями;
- загрузить датасет в pandas DataFrame;
- проверить пропуски и отфильтровать строки по условию.

## Предварительные знания

Достаточно понимать переменные, условия, циклы и запуск Python-скрипта.

:::tip
Выполняйте задания небольшими шагами и сохраняйте вывод каждой проверки: он понадобится для итогового отчёта.
:::
...
- Смешивать загрузку данных, преобразования и вывод в одном большом блоке.
- Игнорировать результат `info()` и проверку пропущенных значений.
- Фильтровать DataFrame без явных условий и не проверять число полученных строк.

## Результат

Сохраните рабочий код и отчёт с выводом ключевых проверок: структура DataFrame, пропуски, отфильтрованные записи, минимум, максимум и средние значения признаков.

<SelfCheck
  questions={[
    { question: "Что показывает DataFrame.info()?", answer: "Размер таблицы, названия столбцов, число непустых значений и типы данных." },
    { question: "Как посчитать пропуски по столбцам?", answer: "Вызвать data.isna().sum()." },
    { question: "Зачем проверять результат фильтрации?", answer: "Чтобы убедиться, что условия сформулированы верно и выборка не оказалась неожиданно пустой." }
  ]}
/>
