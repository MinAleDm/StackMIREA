...
Разберём, как разделить данные, отображение и обработку пользовательских действий между Model, View и Controller.

## После материала вы сможете

- объяснить ответственность Model, View и Controller;
- связать состояние модели с отображением через контроллер;
- заметить смешивание бизнес-логики и интерфейса;
- собрать небольшое Java-приложение по MVC.

## Предварительные знания

Понадобятся классы, методы, конструкторы, инкапсуляция и базовый синтаксис Java.
...
Model хранит состояние, View отвечает за вывод, а Controller принимает действие и координирует изменение модели и обновление интерфейса.

:::warning
Не помещайте бизнес-правила во View. Иначе интерфейс становится трудно переиспользовать и тестировать.
:::

## Реализация

Ниже решение разбито по ролям MVC и исходным файлам. Сначала проследите зависимости между компонентами, затем запускайте демонстрационные классы.
...
- View напрямую изменяет поля Model и обходит Controller.
- Controller одновременно хранит данные, рисует интерфейс и выполняет бизнес-правила.
- Компоненты создаются внутри методов и поэтому их трудно заменить в тесте.

## Самостоятельное задание

Добавьте к примеру редактирование одного поля Model. Изменение должно проходить через Controller, а View — только отображать новое состояние.

<SelfCheck
  questions={[
    { question: "За что отвечает Controller?", answer: "Он принимает действие, изменяет Model и инициирует обновление View." },
    { question: "Должна ли Model зависеть от View?", answer: "Нет. Model хранит состояние и правила предметной области независимо от способа отображения." },
    { question: "Зачем разделять роли?", answer: "Так компоненты проще тестировать, заменять и развивать независимо." }
  ]}
/>
